...
// Global notification and message data
let userNotifications = [];
let userMessages = [];

// Initialize notifications and messages on page load
async function initNotificationsAndMessages() {
    try {
        // Load notifications
        const notifResponse = await fetch('/api/notifications/get_notifications.php', {
            credentials: 'include'
        });
        
        if (notifResponse.ok) {
            const notifData = await notifResponse.json();
            userNotifications = notifData.notifications || [];
    ...
    try {
        ...
        const msgResponse = await fetch('/api/messages/list_conversations.php', {
            credentials: 'include'
        });
        
        if (msgResponse.ok) {
            const msgData = await msgResponse.json();
            userMessages = msgData.conversations || [];
            updateMessageBadge();
        }
    } catch (error) {
...
function loadNotificationsDropdown() {
    ...
    if (notifications.length === 0) {
        content.innerHTML = '<div class="empty-state-dropdown">No notifications yet</div>';
        return;
    }
...
function loadMessagesDropdown() {
    ...
    if (messages.length === 0) {
        content.innerHTML = '<div class="empty-state-dropdown">No messages yet</div>';
        return;
    }
    
    content.innerHTML = messages.map(conv => `
        <div class="message-item ${conv.unread_count > 0 ? 'unread' : ''}">
            <div class="notif-icon">💬</div>
            <div class="notif-content">
                <p class="notif-text"><strong>${escapeHtml(conv.other_user_name || 'User')}</strong></p>
                <p class="notif-text">${escapeHtml(conv.last_message || '')}</p>
                <span class="notif-time">${formatTimeAgo(conv.last_message_time)}</span>
            </div>
        </div>
    `).join('');
...
function updateMessageBadge() {
    ...
    const unreadCount = (userMessages || []).reduce((sum, conv) => sum + (conv.unread_count || 0), 0);
...
document.addEventListener('click', function(e) {
    ...
    if (!e.target.closest('.message-dropdown-wrapper')) {
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initNotificationsAndMessages);
} else {
    initNotificationsAndMessages();
}
